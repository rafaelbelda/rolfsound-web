...
def _find_ytdlp() -> str:
    """Resolve o binário do yt-dlp: primeiro ao lado do Python atual (venv),
    depois no PATH. Rodar via .venv sem ativar deixava 'yt-dlp' inalcançável."""
...
def get_metadata(track_id: str) -> dict | None:
    ...
    cmd = [YTDLP_BIN, "--dump-json", "--no-download", "--no-call-home", url]
    try:
