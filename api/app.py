...
def create_app() -> FastAPI:
    ...
    # Discovery (YouTube via yt-dlp) é exclusivo de conta admin — gate no servidor.
    app.include_router(search.router,     prefix="/api", dependencies=[Depends(require_admin)])
    app.include_router(downloads.router,  prefix="/api", dependencies=[Depends(require_admin)])
    ...
    # ─── APP SHELL ───
    # SPA de página única: qualquer rota fora de /api e /static devolve o
    # index.html da raiz (o app novo, renderizado a partir de /api/bootstrap.js).
